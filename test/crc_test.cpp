...
TEST(TestCRC, OnlyZero) {
  uint8_t input = 0x00;
  uint8_t result = validation::CRC8WCDMA(&input, 1);
...
TEST(TestCRC, MultiValueIdentical) {
  ...
  uint8_t result = validation::CRC8WCDMA(input, sizeof(input) / sizeof(input[0]));
...
TEST(TestCRC, ConsecutiveExecution) {
  ...
  EXPECT_EQ(validation::CRC8WCDMA(input, size), 0x8B);
  EXPECT_EQ(validation::CRC8WCDMA(input, size), 0x8B);
  EXPECT_EQ(validation::CRC8WCDMA(input, size), 0x8B);
  EXPECT_EQ(validation::CRC8WCDMA(input, size), 0x8B);
}
